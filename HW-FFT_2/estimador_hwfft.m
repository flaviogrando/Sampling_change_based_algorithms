...
function [Sa, Sb, Sc, Aa, Ab, Ac, phia, phib, phic] = estimador_hwfft(Va_seg, Vb_seg, Vc_seg, Fs, f0, freq_final)
...
for i=1:num_wins
    ...
    n=0:passo(i):N*passo(i)-passo(i);
    
    for k=0:N-1
        Sa(i,k+1) = 2*(sum(Va_seg(i,:).*exp(-1i*theta*n*k)))/N;
        Sb(i,k+1) = 2*(sum(Vb_seg(i,:).*exp(-1i*theta*n*k)))/N;
        Sc(i,k+1) = 2*(sum(Vc_seg(i,:).*exp(-1i*theta*n*k)))/N;
    end
